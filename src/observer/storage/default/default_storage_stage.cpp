...
void DefaultStorageStage::handle_event(StageEvent *event) {
  ...
  switch (sql->flag)
  {
    case SCF_INSERT: { // insert into
      ...
      for (int i = 1; i < inserts.insert_num; i++) {
        if (inserts.offsets[i] - inserts.offsets[i - 1] != inserts.offsets[0]) {
          rc = RC::SQL_SYNTAX;
        }
      }
      if (rc == RC::SUCCESS) {
    ...
    case SCF_CREATE_INDEX: {
      ...
      if (create_index.isUnique == 1) {
        rc = handler_->create_unique_index(current_trx, current_db, create_index.relation_name, create_index.index_name,
                                    create_index.attr_len, create_index.attribute_name);
      }else { // create multi index
        rc = handler_->create_index(current_trx, current_db, create_index.relation_name, create_index.index_name,
                                           create_index.attr_len, create_index.attribute_name);
      }
      snprintf(response, sizeof(response), "%s\n", rc == RC::SUCCESS ? "SUCCESS" : "FAILURE");
    }
