...
RC DefaultHandler::create_index(Trx *trx, const char *dbname, const char *relation_name,
                                   const char *index_name, const int len, char * const attribute_name[]){
  Table *table = find_table(dbname, relation_name);
  if (nullptr == table) {
    return RC::SCHEMA_TABLE_NOT_EXIST;
  }
  return table->create_index(trx, index_name, attribute_name, len);
}

RC DefaultHandler::create_unique_index(Trx *trx, const char *dbname, const char *relation_name,
                                const char *index_name, const int len, char * const attribute_name[]) {
  Table *table = find_table(dbname, relation_name);
  if (nullptr == table) {
    return RC::SCHEMA_TABLE_NOT_EXIST;
  }
  return table->create_unique_index(trx, index_name, attribute_name, len);
}
...
RC DefaultHandler::update_record(Trx *trx, const char *dbname, const char *relation_name, const char *attribute_name, const Value *value,
  ...
  CompositeConditionFilter conditionFilter;
  RC rc = conditionFilter.init(*table, conditions, condition_num);
  if (rc != RC::SUCCESS) {
    return rc;
  }
  return table->update_record(trx, attribute_name, value, &conditionFilter, updated_count);
}
